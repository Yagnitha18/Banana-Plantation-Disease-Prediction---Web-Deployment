...
class Robot
    ...
    def avoid_obstacles(self,point_cloud,dt):
       ...
       if len(point_cloud) > 1:
           ...
           if closest_obs[1] < self.min_obs_dist and self.count_down > 0:
               ...
               self.count_down = 5
               self.move_forward()
    def move_backward(selfself):
       self.vr = - self.minspeed
       self.vl = - self.minspeed/2
    def move_forward(selfself):
        self.vr = - self.minspeed
    ...
    class Graphics:
        def __init__(self, dimensions, robot_image_path, map_img _path):
            pygame.init()
            self.black = (0, 0, 0)
        ...
        def draw_robot(selfself, x, y, heading):
        ...
        class Ultrasonic:
            ...
            def sense_obstacles(self, x, y, heading):
                ...
                for angle in np.linspace(start_space, finish_angle, 10, false):
                    x2 = x1 + self.sensor_range[0] * math.cos(angle)
                    y2 = y1 - self.sensor_rangw[0] * math.sin(angle)
